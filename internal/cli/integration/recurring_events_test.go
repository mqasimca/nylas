...
func TestRecurringEventOperations(t *testing.T) {
	...
	t.Run("CreateRecurringEvent", func(t *testing.T) {
		...
		if err != nil {
			// Skip if calendar not found or not supported
			if assert.Contains(t, err.Error(), "Not Found") || assert.Contains(t, err.Error(), "not found") {
				t.Skipf("Calendar not available or not supported: %v", err)
			}
